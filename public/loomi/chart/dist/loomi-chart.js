...
let LoomiChart = class LoomiChart extends LoomiElement {
    ...
    async chartCanvas(type = "image/png") {
        ...
        if (!ctx)
            throw new Error("Unable to create chart export canvas");
        if (type === "image/jpeg") {
    ...
    async canvasBlob(canvas, type, quality) {
    ...
    dataCsv() {
        ...
        return [
            header.join(","),
            ...this.data.map((point) => header.map((key) => esc(point[key])).join(",")),
        ].join("\n");
    }
    ascii(value) {
    ...
    async exportAs(format) {
        ...
        this.downloadBytes(`${base}.pdf`, [pdf], "application/pdf");
    }
...
__decorate([
    property({ attribute: "series2-type" })
], LoomiChart.prototype, "series2Type", void 0);
__decorate([
...
__decorate([
    property({ type: Boolean })
], LoomiChart.prototype, "exportable", void 0);
__decorate([
    state()
...
__decorate([
    state()
], LoomiChart.prototype, "exportMenuOpen", void 0);
