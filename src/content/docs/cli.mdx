...
`@loomdev-pro/cli` is the command-line tool for [LoomiUI Pro](/pro/licensing/). Use it to scaffold production-ready starter apps, apply [theme presets](/pro/themes/), add Pro block guides, and keep your codebase in sync as [templates](/pro/templates/) and packages change.
...
2. **Node.js 18 or newer** — `@loomdev-pro/cli` is a Node package.
...
3. **Pro registry access** — add your account token to `.npmrc` so packages
  under `@loomdev-pro/*` can be installed.

<p>&nbsp;</p>
The CLI package is `@loomdev-pro/cli`.
...
<loomi-accordion>
  ...
  <loomi-accordion-item>
    ...
    Ensure your layout file imports `@loomdev-pro/themes/css/<preset>.css` (for example,
