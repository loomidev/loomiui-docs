...
let LoomiChatWindow = class LoomiChatWindow extends LoomiElement {
    constructor() {
        ...
        this.onAttachmentPicked = (type) => (event) => {
            const input = event.currentTarget;
            const files = Array.from(input.files ?? []);
            if (!files.length)
                return;
            this.dispatchEvent(new CustomEvent(type === "file" ? "attach-file" : "attach-picture", {
                bubbles: true,
                composed: true,
                detail: { files },
            }));
    ...
    chooseAttachment(type) {
        ...
        if (type === "file")
            ...
            this.pictureInputEl?.click();
        else
            this.dispatchEvent(new CustomEvent("add-user", { bubbles: true, composed: true }));
    }
    renderHeaderAvatars() {
...
__decorate([
    query(".loomi-chat-file-input")
], LoomiChatWindow.prototype, "fileInputEl", void 0);
__decorate([
    query(".loomi-chat-picture-input")
], LoomiChatWindow.prototype, "pictureInputEl", void 0);
